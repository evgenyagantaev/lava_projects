...
const stdpCtx = {
  ...
  eligibility: stdpCanvases.eligibility.getContext("2d"),
  reward: stdpCanvases.reward.getContext("2d"),
  weight: stdpCanvases.weight.getContext("2d"),
};

const rewardIndicators = {
  A: document.getElementById("reward-a-indicator"),
  B: document.getElementById("reward-b-indicator"),
};
  ...
  state.ws.onclose = () => {
    ...
    state.ws.onmessage = (ev) =>
    {
      ...
      cards.forEach((_, i) =>
      {
        ...
        {
          if (buf[k].length > state.maxBuffer)
          {
            buf[k].splice(0, buf[k].length - state.maxBuffer);
          }
        });
        if (buf.inputsDetail.length > state.maxBuffer)
        {
          buf.inputsDetail.splice(0, buf.inputsDetail.length - state.maxBuffer);
        }
      ...
      // R-STDP traces
      const r = state.rstdp;
      const pre = frame.pre_trace ?? 0;
      const elig = frame.eligibility || [0, 0];
      const reward = frame.reward || [0, 0];
      const weight = frame.weight || [
        r.weightA[r.weightA.length - 1] || 0,
        r.weightB[r.weightB.length - 1] || 0,
      ];

      r.pre.push(pre);
      r.eligA.push(elig[0] ?? 0);
      r.eligB.push(elig[1] ?? 0);
      r.rewardA.push(reward[0] ?? 0);
      r.rewardB.push(reward[1] ?? 0);
      r.weightA.push(weight[0]);
      r.weightB.push(weight[1]);

      ["pre", "eligA", "eligB", "rewardA", "rewardB", "weightA", "weightB"].forEach((k) =>
      {
        const arr = r[k];
        if (arr.length > state.maxBuffer)
        {
          arr.splice(0, arr.length - state.maxBuffer);
        }
      });
      r.step = r.pre.length - 1;

      // Reward indicators
      if (rewardIndicators.A)
      {
        if (reward[0] && reward[0] > 0)
        {
          rewardIndicators.A.classList.add("active");
        }
        else
        {
          rewardIndicators.A.classList.remove("active");
        }
      }
      if (rewardIndicators.B)
      {
        if (reward[1] && reward[1] > 0)
        {
          rewardIndicators.B.classList.add("active");
        }
        else
        {
          rewardIndicators.B.classList.remove("active");
        }
      }
    };
}
...
function drawTrace(key, data, color, dashed = false)
{
  const ctx = stdpCtx[key];
  const canvas = stdpCanvases[key];
  ...
  for (let j = 0; j < view; j++)
  {
    const idx = (state.rstdp.step - j + data.length) % data.length;
...
function drawDualTrace(key, dataA, dataB, colorA, colorB)
{
  const ctx = stdpCtx[key];
  const canvas = stdpCanvases[key];
  const len = Math.min(dataA.length, dataB.length);
  if (!canvas || !ctx || !len) return;
  ...
  const scale = (h - marginY * 2) / Math.max(maxV, 1e-6);
  const dx = state.dx;
  const view = Math.min(state.viewSamples, len);
...
function drawWeight()
{
  ...
  const wA = state.rstdp.weightA;
  const wB = state.rstdp.weightB;
  ...
  const baseY = h - marginY;
  // Fixed vertical scale for synaptic weight:
  // we assume weights are clipped to [0, 1] on the backend side.
  const minV = 0.0;
  const maxV = 1.0;
...
function tick(ts)
{
  ...
  drawTrace("pre", state.rstdp.pre, "#ff5c5c");
  drawDualTrace("eligibility", state.rstdp.eligA, state.rstdp.eligB, "#4db3ff", "#ffa24d");
  drawDualTrace("reward", state.rstdp.rewardA, state.rstdp.rewardB, "#4dff8f", "#ffa24d");
  drawWeight();
